...
- Loads authors' CSV dataset from dataset/data (Sudan 2013, Jordan 2014/2015, Rwanda 2015)
...
- Uses class-standard splits: 80/10/10 (train/val/test)
- Saves per-site summaries and plots.
"""
...
def resolve_csv_dir():
    try:
        ...
        script_dir = Path.cwd()
    candidates = [
        script_dir / 'dataset' / 'data',
        script_dir / 'implementation_project' / 'dataset' / 'data',
        Path.home() / 'workspace/scientific-machine-learning/implementation_project/dataset/data',
    ]
    for p in candidates:
        if p.exists():
            return str(p)
    raise FileNotFoundError('Could not locate dataset/data folder')
...
def main():
    ...
    cm = confusion_matrix(yte_c, (p_te_lr >= 0.5).astype(int))
    cls_metrics = {
        'accuracy': accuracy_score(yte_c, (p_te_lr >= 0.5).astype(int)),
        'balanced_accuracy': balanced_accuracy_score(yte_c, (p_te_lr >= 0.5).astype(int)),
        'f1': f1_score(yte_c, (p_te_lr >= 0.5).astype(int), zero_division=0),
        'roc_auc': roc_auc_score(yte_c, p_te_lr) if len(np.unique(yte_c)) == 2 else float('nan'),
        'avg_precision': average_precision_score(yte_c, p_te_lr),
    ...
    # Hyperparameter search (optional): simple grid over hidden/lr/dropout/pos_weight_scale
